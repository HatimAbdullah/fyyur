...
def search_venues():
  ...
  searchable = request.form.get('search_term', '')
  results = db.session.query(Venue).filter(Venue.name.ilike(f'%{searchable}%')).all()
  data = []
...
def show_venue(venue_id):
  ...
  previous_shows = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time<datetime.now()).all()

  frontend_future = []
  frontend_previous = []

  for show in future_shows:
    frontend_future.append({
      "name": show.name,
  ...
  venue_details = {
    ...
    "past_shows": frontend_previous,
    "upcoming_shows": frontend_future,
    "past_shows_count": len(frontend_previous),
    "upcoming_shows_count": len(frontend_future),
  }
...
def create_venue_submission():
  ...
  try: 
    venue = Venue(name=request.form['name'], city=request.form['city'], state=request.form['state'], address=request.form['address'], phone=request.form['phone'], genres=request.form.getlist('genres'), facebook_link=request.form['facebook_link'], image_link=request.form['image_link'], website=request.form['website'], seeking_talent=True if 'seeking_talent' in request.form else False , seeking_description=request.form['seeking_description'])
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error_flag: 
    flash(f'somtn is off venue {venue_id} is immortal.')
  if not error_flag: 
    flash(f'you will never see Venue {venue_id} again')
...
def search_artists():
  ...
  searchable = request.form.get('search_term', '')
  results = db.session.query(Artist).filter(Artist.name.ilike(f'%{searchable}%')).all()
  data = []
...
def edit_artist_submission(artist_id):
  ...
  try: 
    ...
    db.session.close()
  if error_flag: 
    flash('that is a real one that wont change gtfoh')
  if not error_flag: 
    flash('yeah i knew they would changed smh')
...
def edit_venue_submission(venue_id):
  ...
  try: 
    ...
    db.session.close()
  if error_flag: 
    flash(f'the venue is refusing to change, leave it be')
  if not error_flag: 
    flash(f'you changed the venue, hope you are happy')
...
def create_artist_submission():
  ...
  error_flag = False
  try: 
    artist = Artist(name=request.form['name'], city=request.form['city'], state=request.form['state'], phone=request.form['phone'], genres=request.form.getlist('genres'), facebook_link=request.form['facebook_link'], image_link=request.form['image_link'], website=request.form['website'], seeking_venue=True if 'seeking_venue' in request.form else False , seeking_description=request.form['seeking_description'])
...
def create_show_submission():
  ...
  error_flag = False
  try: 
    show = Show(name=request.form['name'], artist_id=request.form['artist_id'], venue_id=request.form['venue_id'], start_time=request.form['start_time'])
    db.session.add(show)
    db.session.commit()
  except: 
    error_flag = True
